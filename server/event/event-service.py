	...
	for session in singleSessionList:
		...
		if (startDate <= today <= endDate):
			if (startTime <= currentTime <= endTime):
				live = True
		singleDict = {"eventName" : session[2], "eventType" : session[5], "startTime" : str(session[8]), "startDate" : str(session[6]), "endTime" : str(session[9]), "endDate" : str(session[7]), "live" : live, "completed" : completed, "eventCode" : session[1], "eventID" : session[0]}
		# print("\n")
...
def multiSessions(multipleSessionList, status):
	...
	for event in eventList:
		...
		if (nextSession is None):
			...
			multiDict = {"eventName" : eventCopy[len(eventCopy) - 1][2], "eventType" : eventCopy[len(eventCopy) - 1][5], "startTime" : str(eventCopy[len(eventCopy) - 1][8]), "startDate" : str(eventCopy[len(eventCopy) - 1][6]), "endTime" : str(eventCopy[len(eventCopy) - 1][9]), "endDate" : str(eventCopy[len(eventCopy) - 1][7]), "live" : live, "completed" : completed, "eventCode" : eventCopy[len(eventCopy) - 1][1], "eventID" : eventIDList}
			# print("hheya")
			...
			multiDict = {"eventName" : nextSession[2], "eventType" : nextSession[5], "startTime" : startTimeList, "startDate" : startDateList, "endTime" : endTimeList, "endDate" : endDateList, "live" : live, "completed" : completed, "eventCode" : nextSession[1], "eventID" : eventIDList}
			print("\n")
			print("multiDict : ", multiDict)
